...
/**
 * Fill in `email` for a roster. Emails live on `auth.users`, which PostgREST
 * can't read, so this pages through the service-role admin API — call it
 * only where the address is actually shown (Configuración → Equipo), not on
 * every roster read (/hoy, asignación).
 */
export async function attachEmails(roster: RosterMember[]): Promise<RosterMember[]> {
  ...
  return roster.map((m) => ({ ...m, email: emailById.get(m.id) ?? m.email }));
}
